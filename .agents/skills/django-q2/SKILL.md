...
- `Q_CLUSTER["name"]` comes from `Q_CLUSTER_NAME`, which defaults to
  `<LOCAL_INSTANCE_ID>-q`. Keep web and worker processes on the same value.
...
When two same-slug clones intentionally share one Redis service, set unique
`LOCAL_INSTANCE_ID`, `CACHE_KEY_PREFIX`, and `Q_CLUSTER_NAME` values per clone.
Do not let two unrelated repos share the same cluster name.
